  ...
  const fetchUserEmail = async (
    ...
    try {
      ...
      const usersWithEmails = await Promise.all(data.map(async (item) => {
        let email = item.actor_email || '';
        let agreementName = 'N/A';

        // Attempt to enrich via Consent_Record if applicable
        if (item.entity_id) {
          ...
          if (consentData && consentData.length > 0) {
            ...
            if (agreementData?.agreement_name) agreementName = agreementData.agreement_name;
          } else {
            // Check if entity_id is directly a user_id
            const { data: directUser } = await supabase
              .from('User')
              .select('email')
              .eq('user_id', item.entity_id)
              .single();
        ...
        return {
          ...
          consent_status: item.consent_status || 'N/A'
        };
      }));

      return { data: usersWithEmails, totalCount: count };
      ...
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          <Heading
            ...
            description="Immutable compliance audit trails under India's DPDP Act"
          />
{/* 
          <Link
